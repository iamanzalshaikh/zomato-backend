...
In **development**, new riders are auto-**approved**. Production riders stay `pending` until admin approves.
...
| PATCH | `/pickup-order/:orderId` | `PICKED_UP` → `ON_THE_WAY` |
...
8. `PATCH /riders/location` (repeat while delivering)  
9. `PATCH /riders/complete-delivery/:orderId`  
10. `GET /riders/earnings`  
